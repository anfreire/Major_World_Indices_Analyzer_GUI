...
icon = ImageTk.PhotoImage(Image.open("assets/icon.ico"))
...
def proceed_period():
	...
	try:
		...
		open_f4("Select a Index Fund from the list!")
		return 
...
def proceed_results():
	try:
		validate_period(entry_1_f3.get(), entry_2_f3.get())
	except ValueError:
		open_f4("Enter a number for the amount,\nand a period for the season.\nEx: 5 Days, 15 Months, 30 Years")
		return
	results()
	forget_period()
	process_stocks()
...
def validate_option():
	if len(listbox_f2.curselection()) == 0:
		raise ValueError
...
def save_xlsl_36indexs(name):
	...
	if len(name) > 31:
		name = name[0:31]
	open_f4(save_xlsl_data(df, name))
...
def	process_stocks():
	try:
		...
		open_f4("Unable to get data from this Stock.\nSorry for the Inconvinience. :( \nTry another one!")
		return
...
def save_xlsx_one_index():
	try:
		...
		open_f4(f"{name_var}.xlsx  Saved!")
	except:
		open_f4("An unknown error occurred.\nPlease restart the program.\nSorry for any inconvenience")
		return
...
button_2_f3.config(command=lambda:proceed_results())
